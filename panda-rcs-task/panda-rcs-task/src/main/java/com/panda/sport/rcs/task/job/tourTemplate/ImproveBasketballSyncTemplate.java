...
@Component
@Slf4j
public class ImproveBasketballSyncTemplate {
    private final  RedisClient redisClient;

    private final  ProducerSendMessageUtils producerSendMessageUtils;

    public ImproveBasketballSyncTemplate( RedisClient redisClient, ProducerSendMessageUtils producerSendMessageUtils) {
        ...
        if(!CollectionUtils.isEmpty(querylist)){
         ...
         /*   int sendSizePercent = getSendSizePercent(querylist,unSendList,lastData);
            ...
            if(sendSizePercent>50 ){
                //超过50%，放弃本次执行
                log.info("::{}::-结束同步{}篮球赛事模板,上一次执行的数据有{}%未执行完成,占总数的{}%，等下一次执行" ,linkId,DateUtil.format(new Date(),"yyyy-MM-dd HH:mm:ss"),sendSizePercent,unSendSizePercent);
            }else{*/
            // 第六步：每次推送不存在的数据D  取消4，5步，防止分时同步停顿
            sendMsg(unSendList);
            // 第五步：更新缓存
            redisClient.setExpiry(redis_data_key,sendData,300L);
            log.info("::{}::-结束同步篮球赛事模板,结束时间：{}" ,linkId,DateUtil.format(new Date(),"yyyy-MM-dd HH:mm:ss"));
//            }
        }else{
            log.info("::{}::-结束同步篮球赛事模板,当前时间点未有数据待同步" ,linkId);
        }
    }
    ...
    private void sendMsg(List<RcsTournamentTemplateComposeModel> unSendList) {
        ...
        for(int i=0;i<unSendList.size();i=i+200){
            ...
            producerSendMessageUtils.sendMessage("BASKETBALL_SYNC_TEMPLATE","BASKETBALL_SYNC_TEMPLATE","",subList);
        }
    }
