...
public class MongoServiceImpl<T> implements MongoService<T> {
    ...
    @Override
    public List<MarketCategory> queryMarketCategory(String matchId, Long playId){
        Criteria criteria = Criteria.where("matchId").is(matchId).and("id").is(playId);
        List<MarketCategory> marketCategoryList = mongoTemplate.find(new Query().addCriteria(criteria), MarketCategory.class);
