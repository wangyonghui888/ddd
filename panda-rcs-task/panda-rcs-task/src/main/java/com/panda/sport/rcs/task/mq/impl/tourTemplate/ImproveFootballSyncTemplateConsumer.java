...
@RocketMQMessageListener(
        topic = "FOOTBALL_SYNC_TEMPLATE",
        consumerGroup = "FOOTBALL_SYNC_TEMPLATE",
        messageModel = MessageModel.CLUSTERING,
        consumeMode = ConsumeMode.ORDERLY)
public class ImproveFootballSyncTemplateConsumer extends RcsConsumer<List<RcsTournamentTemplateComposeModel>>  {
    ...
    protected String getTopic() {
        return "FOOTBALL_SYNC_TEMPLATE";
    }

    @Override
    public Boolean handleMs(List<RcsTournamentTemplateComposeModel> list) {
        String linkId= "FOOTBALL_SYNC_TEMPLATE" + UUID.randomUUID().toString().replace("-", "") ;
        log.info("::{}::-Consumer开始执行足球定时任务同步模板:{}",linkId, DateUtil.format(new Date(),"yyyy-MM-dd HH:mm:ss"));
        try {
            commonRunMethod.handleTemplateDataThread(linkId,list);
        }catch (Exception e) {
