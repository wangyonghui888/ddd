    ...
    layui.use('table', function () {
        ...
        tableIns = table.render({
            ...
            cols: [[
                ...
                , {
                    field: 'infoStatus', title: '订单详细状态', align: 'left', templet: function (d) {
                    	// 0：待处理 1：赛前订单接单通过   2：赛前订单拒单（需要备注原因）\r\n           
//                    	3：MTS接单   4：MTS拒单    5：业务拒单  6：接拒单处理中  7：MTS处理中\r\n           
//                    	8：接拒单成功  9：接拒单拒绝（原因）\r\n
                        if (d.infoStatus == '0') {
                            return '待处理'
                        }
                ...
                , {field: 'rec_val', title: '矩阵值', align: 'left'}
                , {
                ...
                , {
                    field: 'accOrderStatus', title: '接拒单状态', align: 'left', templet: function (d) {
                    	// 0 待处理 1 接单  2拒单
                        if (d.accOrderStatus == '0') {
                            return '待处理'
                        }
                        if (d.accOrderStatus == '1') {
                            return '接单'
                        }
                        if (d.accOrderStatus == '2') {
                            return '拒单'
                        }
                    }
                }
                , {
                    field: 'mode', title: '模式', align: 'left', templet: function (d) {
                    	// 0 自动 1 手动
                        if (d.mode == '0') {
                            return '自动'
                        }
                ...
                , {
                    field: 'handleStatus', title: '接拒单处理状态', align: 'left', templet: function (d) {
                        if (d.handleStatus == '0') {
                            return '未处理'
                        }
                        if (d.handleStatus == '1') {
                ...
                , {field: 'accTime', title: '接单时间', align: 'left'}
            ]],
