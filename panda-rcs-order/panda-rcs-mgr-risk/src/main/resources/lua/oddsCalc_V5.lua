...
if tonumber(sportId) ~= 1 then
    if balanceOption == 1 then
        redis.call("DEL", orderAmountKey);
    else
        redis.call("DEL", orderAmountPlusKey);
    end;
end;
...
if isChangeOdds == 1 then
	if isMultipleJumpOdds == 1 and oddChangeRule == 1 and tonumber(sportId) ~= 1 then
        local twoLevel = (diff - (diff % homeSecondLevel)) / homeSecondLevel;
        local oneLevel = (diff % homeSecondLevel) / homeOneLevel;
		-- 2级*10000 用于跳赔计算1、2级分别跳了多少次
		changeLevel = twoLevel*10000 + oneLevel*100;
		-- 清空钱
		 redis.call("DEL", orderAmountKey);
		 redis.call("DEL", orderAmountPlusKey);

		-- 当前投注项的钱为剩下的钱
		local amount = (diff % homeSecondLevel) % homeOneLevel;
		if balanceOption == 1 then
			redis.call("HINCRBYFLOAT", orderAmountPlusKey, playOptionId, amount - amount % 1);
		else
			redis.call("HINCRBYFLOAT", orderAmountKey, playOptionId, amount - amount % 1);
		end;
	else
		-- 清空钱
		 redis.call("DEL", orderAmountKey);
...
return { lockVal, isChangeOdds, changeLevel, tostring(currentOptionAmount), tostring(allAmount), isHome ,diff};