package com.panda.sport.rcs.mgr.paid.intef.impl;

import com.alibaba.fastjson.JSONArray;
import com.alibaba.fastjson.JSONObject;
import com.panda.sport.data.rcs.dto.ExtendBean;
import com.panda.sport.rcs.common.FileReadUtils;
import com.panda.sport.rcs.constants.RedisKeys;
import com.panda.sport.rcs.core.cache.client.RedisClient;
import com.panda.sport.rcs.exeception.RcsServiceException;
import com.panda.sport.rcs.log.LogContext;
import com.panda.sport.rcs.pojo.RcsBusinessUserPaidConfig;
import com.panda.sport.rcs.mgr.paid.annotion.Order;
import com.panda.sport.rcs.mgr.paid.intef.AmountValidateAdapter;
import com.panda.sport.rcs.mgr.wrapper.impl.RcsRectanglePlayServiceImpl;
import lombok.extern.slf4j.Slf4j;
import org.springframework.beans.factory.annotation.Autowired;

import java.math.BigDecimal;
import java.util.*;

/*
 * 用户单场最高赔付
 */
@Order(value = 3)
@Slf4j
public class UserMatchMaxPaid extends AmountValidateAdapter{
	
    private RedisClient redisClient;
    
	@Autowired
	private RcsRectanglePlayServiceImpl playService;
	
	private String shakey;
	
	@Autowired
	public UserMatchMaxPaid(RedisClient redisClient){
		this.redisClient = redisClient;
		String text = FileReadUtils.readFileContent("lua/userMatch.lua");

		log.info("lua 脚本内容text:{}",text);
		shakey = redisClient.scriptLoad(text);
		if(shakey == null) 
			throw new RcsServiceException("lua脚本加载失败");
	}

	@Override
	public Long getSurplusAmount(ExtendBean order, Long[][] rec) {
		//获取矩阵最大和缓存累加的最大赔付
		Long maxPaidScore = getRecMaxPaid(order, rec);
		//获取用户维度赛事不能用比分推算的最大赔付
		Long noScorePlayAmount = playService.queryMatchNoScorePlay(order);
		RcsBusinessUserPaidConfig userConfig = getUserConfig(order);
		if(userConfig == null ) {
			log.warn("::{}:: requestId:{},赛事维度配置获取失败，ExtendBean:{}",order.getOrderId(),LogContext.getContext().getRequestId(),JSONObject.toJSONString(order));
			return -1L;
		}

        log.info("::{}:: requestId:{},UserMatchMaxPaid, getUserMatchPayVal:{},maxPaidScore:{},noScorePlayAmount:{}",order.getOrderId(),LogContext.getContext().getRequestId(),userConfig.getUserMatchPayVal(),maxPaidScore,noScorePlayAmount);

        //new BigDecimal(order.getCurrentMaxPaid())
        maxPaidScore = userConfig.getUserMatchPayVal().subtract(new BigDecimal(maxPaidScore)).subtract(new BigDecimal(noScorePlayAmount)).longValue();

		log.info("::{}:: requestId:{},UserMatchMaxPaid, getUserMatchPayVal:{},maxPaidScore:{},noScorePlayAmount:{}",order.getOrderId(),LogContext.getContext().getRequestId(),userConfig.getUserMatchPayVal(),maxPaidScore,noScorePlayAmount);
		
		if(maxPaidScore < 0 ) {
			log.warn("::{}:: requestId:{},赛事维度最大赔付小于0，ExtendBean:{}，maxPaidScore：{}",order.getOrderId(),LogContext.getContext().getRequestId(),JSONObject.toJSONString(order),maxPaidScore);
			return -1L;
		}
		
		return maxPaidScore;
	}
	
	public RcsBusinessUserPaidConfig getUserConfig(ExtendBean order) {
		RcsBusinessUserPaidConfig userConfig = order.getUserConfig();
		if(userConfig == null ) {
			userConfig = configService.getUserPaidConfig(order.getBusId(), order.getUserId());
			if(userConfig == null ) return null;
			order.setUserConfig(userConfig); 
		}
		return userConfig;
	}
	
	/**
	 * 获取矩阵和当前订单矩阵相加的最大赔付值
	 */
	private Long getRecMaxPaid(ExtendBean order, Long[][] rec) {
		Long maxPaidScore = Long.MAX_VALUE;
		if( order.getRecType() == null|| rec == null || order.getRecType() == 1) {
		    //不可用比分推算的
			Map<String, Long> infoMap = redisClient.hGetAll(String.format(RedisKeys.PAID_DATE_USER_REDIS_CACHE , 
					order.getDateExpect(),order.getUserId(),order.getMatchId()),Long.class);
			if(infoMap == null || infoMap.size() <= 0 ) return 0L;
			
			maxPaidScore = Collections.min(infoMap.values());

			log.info("::{}:: requestId:{},UserMatchMaxPaid, 不可用比分推算的:{},maxPaidScore:{}",order.getOrderId(),LogContext.getContext().getRequestId(),infoMap,maxPaidScore);
		}else {//可用比分推算的需要做矩阵累加
			Map<String, Long> infoMap = redisClient.hGetAll(String.format(RedisKeys.PAID_DATE_USER_REDIS_CACHE , 
					order.getDateExpect(),order.getUserId(),order.getMatchId()),Long.class);
			for(int i = 0 ; i < rec.length ; i ++) {
				for(int j = 0 ; j < rec[i].length ; j ++) {
					String key = String.format("%03d", i) + String.format("%03d", j);
					Long oldAmount = infoMap.get(key) == null ? 0l : infoMap.get(key);
					maxPaidScore = Math.min(maxPaidScore, oldAmount + rec[i][j]);
				}
			}
			log.info("::{}:: requestId:{},UserMatchMaxPaid, 可用比分推算的:{},maxPaidScore:{}",order.getOrderId(),LogContext.getContext().getRequestId(),infoMap,maxPaidScore);
		}
		
		return maxPaidScore * -1;
	}
	
	/**
	 * 获取矩阵和当前订单矩阵相加的最大赔付值,做矩阵累加
	 */
	private Long getRecMaxPaidBySave(ExtendBean order, Long[][] rec, Map<String, Object> data) {
		Long maxPaidScore = Long.MAX_VALUE;
		//不可用比分推算的
		if(order.getRecType() == null|| rec == null || order.getRecType() == 1) {
			Map<String, Long> infoMap = redisClient.hGetAll(String.format(RedisKeys.PAID_DATE_USER_REDIS_CACHE , 
					order.getDateExpect(),order.getUserId(),order.getMatchId()),Long.class);
			if(infoMap == null || infoMap.size() <= 0 ) return 0l;
			
			maxPaidScore = Collections.min(infoMap.values());
		}else {//可用比分推算的需要做矩阵累加
			Map<String, Long> updateMap = new HashMap<String, Long>();
			data.put("UserMatchMaxPaid-updateMap", updateMap);
			for(int i = 0 ; i < rec.length ; i ++) {
				for(int j = 0 ; j < rec[i].length ; j ++) {
					String field = String.format("%03d", i) + String.format("%03d", j);
					long result = redisClient.hincrBy(String.format(RedisKeys.PAID_DATE_USER_REDIS_CACHE , 
							order.getDateExpect(),order.getUserId(),order.getMatchId()), field, rec[i][j]);
					updateMap.put(field, rec[i][j]);
					
					if(maxPaidScore > result)  maxPaidScore = result;
				}
			}
		}
		data.put("addVal", maxPaidScore);
		data.put("type", "用户单场最高赔付");
		return maxPaidScore * -1;
	}
	
	public static void main(String[] args) {
		System.out.println(String.format("%03d", 11));
	}
	
	private Long executeUserMatchLua(String key ,Long[][] rec) {
		//要获取的key值
    	List<String> keys = new ArrayList<>();
    	keys.add(key);
    	//传入的参数
    	List<String> args = new ArrayList<>();
    	StringBuilder sb = new StringBuilder();
    	for(int i = 0 ; i < rec.length ; i ++) {
    		for(int j = 0 ; j < rec[i].length ; j ++) {
    			sb.append(",").append("[\"" + String.format("%03d", i) + String.format("%03d", j) + "\"]").append("=").append(rec[i][j]);
    		}
    	}
    	args.add("{" + sb.toString().substring(1) + "}");
    	args.add(rec.length + "");
    	Object ret = redisClient.evalsha(shakey, keys, args);
		ret = ret == null ? Arrays.asList(0) : ret;
		return JSONObject.parseArray(JSONObject.toJSONString(ret)).getLongValue(0) * -1;
	}
	
	@Override
	public Boolean saveOrder(ExtendBean order, Long[][] rec, Map<String, Object> data) {
		String keyLock = String.format(RedisKeys.PAID_DATE_USER_REDIS_CACHE + "_lock" , 
				order.getDateExpect(),order.getUserId(),order.getMatchId());
		Long valueLock = System.currentTimeMillis();
		try {
			log.info("UserMatchMaxPaid  saveOrder start....");

			String hashKey = String.format(RedisKeys.PAID_DATE_USER_REDIS_CACHE , order.getDateExpect(),order.getUserId(),order.getMatchId());
			Long maxPaidScore = executeUserMatchLua(hashKey, rec);
			data.put("UserMatchMaxPaid-updateMap", rec);
//			redisClient.lock(keyLock, valueLock, 30);
//			//对矩阵做redis累加，取出矩阵最大赔付值
//			Long maxPaidScore = getRecMaxPaidBySave(order, rec, data);
			//查询赛事不能用比分推算的最大赔付
			Long noScorePlayAmount = playService.queryMatchNoScorePlay(order);
			
			RcsBusinessUserPaidConfig userConfig = getUserConfig(order);
			log.info("::{}:: UserMatchMaxPaid  noScorePlayAmount is:{},userConfig is:{},maxPaidScore is:{}",order.getOrderId(),noScorePlayAmount,userConfig,maxPaidScore);
			if(userConfig == null ) {
				log.warn("::{}::用户维度配置为空",order.getOrderId());
				throw new RcsServiceException("用户维度配置为空," + JSONObject.toJSONString(order));
			}
			maxPaidScore = userConfig.getUserMatchPayVal().subtract(new BigDecimal(maxPaidScore)).subtract(new BigDecimal(noScorePlayAmount)).longValue();
			log.info("::{}:: UserMatchMaxPaid  maxPaidScore is:{}",order.getOrderId(),maxPaidScore);
			if(maxPaidScore < 0 ) {
				log.warn("::{}::超过用户维度配置",order.getOrderId());
				throw new RcsServiceException("超过用户维度配置," + userConfig.getUserMatchPayVal().toPlainString() + "," + maxPaidScore + "," + noScorePlayAmount + "," + JSONObject.toJSONString(order));
			}
			return true;
		}catch (RcsServiceException e) {
			log.error("::{}::UserMatchMaxPaid error:{}",order.getOrderId(),e.getMessage());
			rollBack(order, rec, data);
			return false;
		}catch (Exception e) {
			log.error("::{}:: UserMatchMaxPaid error:{}",order.getOrderId(),e.getMessage());
			rollBack(order, rec, data);
			return false;
		}finally {
//			redisClient.unlock(keyLock, valueLock);
		}
	}

	@Override
	public void rollBack(ExtendBean order, Long[][] rec, Map<String, Object> data) {
		if(!data.containsKey("UserMatchMaxPaid-updateMap")) {
			if(order.getRecType() == 1 || rec == null) return;
			
			Map<String, Long> dataMap = new HashMap<String, Long>();
			for(int i = 0 ; i < rec.length ; i ++) {
				for(int j = 0 ; j < rec[i].length ; j ++) {
					String field = String.format("%03d", i) + String.format("%03d", j);
					dataMap.put(field, rec[i][j]);
				}
			}
			data.put("UserMatchMaxPaid-updateMap", dataMap);
		}
		
		Map<String, Long> updateMap = (Map<String, Long>) data.get("UserMatchMaxPaid-updateMap");
		for(String key : updateMap.keySet()) {
			redisClient.hincrBy(String.format(RedisKeys.PAID_DATE_USER_REDIS_CACHE , 
					order.getDateExpect(),order.getUserId(),order.getMatchId()), key, -1 * updateMap.get(key));
		}
	}

	@Override
	public void prizeHandle(ExtendBean orderItem) {
		//Long prizeAmount = orderItem.getOrderMoney() - orderItem.getSettleAmount().longValue() ;
		if(orderItem.getRecType() == 0) {
			JSONArray arrays = JSONArray.parseArray(orderItem.getRecVal());
			for(int i = 0 ; i < arrays.size() ; i ++) {
				JSONArray keys = arrays.getJSONArray(i);
				for(int j = 0 ; j < keys.size() ; j ++) {
					String key = String.format("%03d", i) + String.format("%03d", j);
					//矩阵每个数据都加上当前订单的实际赔付
					redisClient.hincrBy(String.format(RedisKeys.PAID_DATE_USER_REDIS_CACHE ,
							orderItem.getDateExpect(),orderItem.getUserId(),orderItem.getMatchId()), key, -1 * keys.getLongValue(j) - orderItem.getProfit());
				}
			}
		}
	}
	
}
