...
public class RedisUtils {
    ...
    public List<String> blpop(final String key, int time) {
        return doExec(jedis -> jedis.blpop(time, key)).orElse(Lists.newArrayList());
    }

    public long rpush(final String key, String... value) {
        return doExec(jedis -> jedis.rpush(key, value)).orElse(0L);
    }
