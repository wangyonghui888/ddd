...
@RocketMQMessageListener(
        topic = "RCS_RISK_REDIS_CACHE_SYNC",
        consumerGroup = "RCS_RISK_REDIS_CACHE_SYNC_TOPIC",
...
public class RedisCacheSyncConfigConsumer implements RocketMQListener<RedisCacheSyncBean>, RocketMQPushConsumerLifecycleListener {
    ...
    public static void main(String[] args) {
        RedisCacheSyncBean redisCacheSyncBean= RedisCacheSyncBean.build("test","magic-test");
        System.out.println(JSONObject.toJSONString(redisCacheSyncBean));
    }
    /**
        ...
        try {
            if (StringUtils.isNotBlank(bean.getSubKey())) {
                ...
                if (StringUtils.isNotBlank(bean.getValue())) {
                    ...
                    log.info("redis缓存同步,SubKey设置数据KEY:{}，expire：{}", JSONObject.toJSONString(bean.getKey()),bean.getExpiry().intValue());
                } else {
                    redisClient.hashRemove(bean.getKey(), bean.getSubKey());
                    log.info("redis缓存同步,SubKey删除数据KEY:{}", JSONObject.toJSONString(bean.getKey()));
                }
            } else {
                if (StringUtils.isNotBlank(bean.getValue())) {
                    //如果有value 直接进行set覆盖 否者del
                    redisClient.setExpiry(bean.getKey(), bean.getValue(), bean.getExpiry());
                    log.info("redis缓存同步,设置数据KEY:{}", JSONObject.toJSONString(bean.getKey()));
                } else {
                    redisClient.delete(bean.getKey());
                    log.info("redis缓存同步,删除数据KEY:{}", JSONObject.toJSONString(bean.getKey()));
                }
            }
        } catch (Exception e) {
            log.error("::{}::redis缓存同步异常:{}",bean.getKey(), JSONObject.toJSONString(bean), e);
        }
    }
}
