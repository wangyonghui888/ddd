...
/**
 * @description: 篮球大小  Forecast计算
 * @author: lithan
 * @date: 2021-1-9 16:48:58
 **/
@Slf4j
@Service("basketMatrixGoalLineService")
public class BasketMatrixGoalLineServiceImpl implements BasketBallForecastService {
    ...
    public List<RcsPredictBasketballMatrix> calculate(OrderBean orderBean, Integer type) {
        ...
        for (Integer i = min; i <= max; i++) {
            ...
            for (Double markValue : marketValueList) {
                ...
                if (i - markValue > 0) {
                    if ("Over".equalsIgnoreCase(orderItem.getPlayOptions())) {//赢
                    ...
                    if ("Under".equalsIgnoreCase(orderItem.getPlayOptions())) {//赢
                        amount = betAmount.multiply(new BigDecimal(String.valueOf(orderItem.getHandleAfterOddsValue())).subtract(new BigDecimal(1))).multiply(new BigDecimal("-1"));
                    } else {
                        amount = betAmount;
                    }
