...
/**
 * @description: 篮球 净胜分   Forecast计算
 * @author: lithan
 * @date:  2021-1-15 14:47:22
 **/
@Slf4j
@Service("basketMatrixVictoryPointsService")
public class BasketMatrixVictoryPointsServiceImpl implements BasketBallForecastService {
...
//                }
            if (i  >= 3) {
                if ("1And3+".equalsIgnoreCase(orderItem.getPlayOptions())) {//赢
                ...
                if ("2And3+".equalsIgnoreCase(orderItem.getPlayOptions())) {//赢
                    amount = betAmount.multiply(new BigDecimal(String.valueOf(orderItem.getHandleAfterOddsValue())).subtract(new BigDecimal(1))).multiply(new BigDecimal("-1"));
                } else {
                    amount = betAmount;
                }
