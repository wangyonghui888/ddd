...
import com.panda.sport.rcs.pojo.vo.api.request.ActualVolumeAO;
import com.panda.sport.rcs.predict.common.PlayIdsEnum;
 ...
 */
@RestController
@RequestMapping(value = "/actualVolume")
@Slf4j
public class ActualVolumeController {
    ...
    public HttpResponse<HashMap<Integer, HashMap<Integer, List<ActualVolumeApiVO>>>> getList(@RequestBody ActualVolumeAO actualVolumeAO) {
        try {
            ...
            List<ActualVolumeApiVO> actualVolumeVOList;
            if (!Arrays.asList(3, 4).contains(matchStatus)) {
                actualVolumeVOList = this.getRcsPredictBetStatisVo(matchId, playIds, matchType);
            } else {
                actualVolumeVOList = rcsPredictBetStatisMapper.getRcsPredictBetStatisVo1(matchId, playIds, matchType);
            }
            //过滤为0的数据
            actualVolumeVOList = actualVolumeVOList.stream().filter(e -> e.getBetNum() > 0).collect(Collectors.toList());
            HashMap<Integer, HashMap<Integer, List<ActualVolumeApiVO>>> map = new HashMap<>();
            if (!CollectionUtils.isEmpty(actualVolumeVOList)) {
                HashMap<String, List<ActualVolumeApiVO>> hashMap = new HashMap<>();
                for (ActualVolumeApiVO actualVolumeVO : actualVolumeVOList) {
                    ...
                    List<ActualVolumeApiVO> actualVolumeVOS = hashMap.get(actualVolumeVO.getMarketId());
                ...
                for (List<ActualVolumeApiVO> list : hashMap.values()) {
                    if (list.size() == 1) {
                        ActualVolumeApiVO actualVolumeVO = list.get(0);
                        ActualVolumeApiVO copyActualVolumeVO = new ActualVolumeApiVO();
                    ...
                    for (ActualVolumeApiVO actualVolumeVO : list) {
                ...
                for (List<ActualVolumeApiVO> list : hashMap.values()) {
                    ...
                    HashMap<Integer, List<ActualVolumeApiVO>> integerListHashMap = map.get(playTimeStage);
                    if (CollectionUtils.isEmptyMap(integerListHashMap)) {
                        integerListHashMap = new HashMap<>();
                    }
                    List<ActualVolumeApiVO> list1 = integerListHashMap.get(Integer.valueOf(actualVolumeVO.getSubPlayId()));
                    ...
                    for (ActualVolumeApiVO actualVolumeVO1 : list) {
     ...
     * @Date: 2020/12/5
     */
    private void dealingWithprofits(HashMap<Integer, HashMap<Integer, List<ActualVolumeApiVO>>> map, Integer matchId, Integer matchType, List<Integer> playIds) {
        //赛事结束
        ...
        if (!CollectionUtils.isEmpty(rcsMatchProfitVos)) {
            for (RcsMatchProfitVo rcsMatchProfitVo : rcsMatchProfitVos) {
                HashMap<Integer, List<ActualVolumeApiVO>> integerListHashMap = map.get(PlayIdsEnum.getPlayIdsEnum(rcsMatchProfitVo.getPlayId().intValue()).getPlayTimeStage());
                List<ActualVolumeApiVO> list = integerListHashMap.get(rcsMatchProfitVo.getPlayId().intValue());
                for (ActualVolumeApiVO actualVolumeVO : list) {
     ...
     */
    @Deprecated
    private List<ActualVolumeApiVO> getRcsPredictBetStatisVo(Integer matchId, List<Integer> playIds, Integer matchType) {
        List<ActualVolumeApiVO> actualVolumeVOList = rcsPredictBetStatisMapper.getRcsPredictBetStatisVo1(matchId, playIds, matchType);
        Map<String, List<ActualVolumeApiVO>> collect = actualVolumeVOList.stream().collect(Collectors.groupingBy(ActualVolumeApiVO::getMarketId));
        collect.forEach((key, item) -> {
            if (item.size() == 1) {
                ActualVolumeApiVO actualVolumeVO = item.get(0);
                ActualVolumeApiVO copyActualVolumeVO = new ActualVolumeApiVO();
                BeanUtils.copyProperties(actualVolumeVO, copyActualVolumeVO);
        ...
        actualVolumeVOList.forEach(item -> {
            String betStatisKey = "rcs:risk:predict:betSatis.match_id.%s.match_type.%s.play_id.%s.sub_play_id.%s.market_id.%s.odds_item.%s.0:0";
            betStatisKey = String.format(betStatisKey, item.getMatchId(), item.getMatchType(), item.getPlayId(), item.getSubPlayId(), item.getMarketId(), item.getOddsItem());
