...
/**
 * @description: 篮球让球 217 准确总分  Forecast计算
 * @author: lithan
 * @date: 2021-1-16 10:32:12
 **/
@Slf4j
@Service("basketBallAccurateService")
public class BasketMatrixAccurateServiceImpl implements BasketBallForecastService {
    ...
    public List<RcsPredictBasketballMatrix> calculate(OrderBean orderBean, Integer type) {
        ...
        //中值
        int middle = basketMatrixService.getMiddleValue(orderItem.getMatchId(), orderItem.getPlayId(), 1);
        int min = middle - 50;
        int max = middle + 50;
        String orderRec = "";
        //计算每个分差 对应的 预期 盈利
        for (Integer i = min; i <= max; i++) {
            ...
            for (Double marketValue : marketValueList) {
                ...
                if (i - marketValue > 0) {
                    if ("Over".equals(orderItem.getPlayOptions())) {//大  注单中奖 玩家赢  庄家输 预期盈利为负  需要乘以负一
                    ...
                    if ("X".equals(orderItem.getPlayOptions())) {//大  注单中奖 玩家赢  庄家输 预期盈利为负  需要乘以负一
                        amount = betAmount.multiply(new BigDecimal(String.valueOf(orderItem.getHandleAfterOddsValue())).subtract(new BigDecimal(1))).multiply(new BigDecimal("-1"));
                    } else {//注单没奖 玩家输  庄家赢 预期盈利为正
                        amount = betAmount;
                    }
                } else {
                    if ("Under".equals(orderItem.getPlayOptions())) {//小   注单中奖 玩家赢  庄家输 预期盈利为负  需要乘以负一
                        amount = betAmount.multiply(new BigDecimal(String.valueOf(orderItem.getHandleAfterOddsValue())).subtract(new BigDecimal(1))).multiply(new BigDecimal("-1"));
                    } else {//注单没奖 玩家输  庄家赢 预期盈利为正
                        amount = betAmount;
                    }
