...
/**
 * @description: 篮球 单双  Forecast计算
 * @author: lithan
 * @date:  2021-1-15 14:47:22
 **/
@Slf4j
@Service("basketMatrixSingleDoubleService")
public class BasketMatrixSingleDoubleServiceImpl implements BasketBallForecastService {
...
//                }
            if (i % 2 == 0) {
                if ("Even".equalsIgnoreCase(orderItem.getPlayOptions())) {//赢
...
//                    //走水
                ...
                if ("Odd".equalsIgnoreCase(orderItem.getPlayOptions())) {//赢
                    amount = betAmount.multiply(new BigDecimal(String.valueOf(orderItem.getHandleAfterOddsValue())).subtract(new BigDecimal(1))).multiply(new BigDecimal("-1"));
                } else {
                    amount = betAmount;
                }
