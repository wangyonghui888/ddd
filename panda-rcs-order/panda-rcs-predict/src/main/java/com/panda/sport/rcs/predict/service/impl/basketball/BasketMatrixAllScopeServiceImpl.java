...
/**
 * @description: 篮球 总分区间-218 Forecast计算
 * @author: lithan
 * @date:  2021-1-15 14:47:22
 **/
@Slf4j
@Service("basketMatrixAllScopeService")
public class BasketMatrixAllScopeServiceImpl implements BasketBallForecastService {
...
//                }
            ...
            String playOption = orderItem.getPlayOptions();
            Double begin =0D;
            Double end =Double.MAX_VALUE;
            if (playOption.indexOf("+") == -1) {
                String arr[] = playOption.split("-");
                begin = Double.valueOf(arr[0]);
                end = Double.valueOf(arr[1]);
            }else {
                begin = Double.valueOf(playOption.replace("+", ""));
            }
