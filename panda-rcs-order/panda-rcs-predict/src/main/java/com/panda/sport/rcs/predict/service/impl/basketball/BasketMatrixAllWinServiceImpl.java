...
/**
 * @description: 全场胜平负 1  Forecast计算
 * @author: lithan
 * @date:  2021-1-15 14:47:22
 **/
@Slf4j
@Service("basketMatrixAllWinService")
public class BasketMatrixAllWinServiceImpl implements BasketBallForecastService {
...
//                }
            //如果买主队
            if (i  > 0) {
                if ("1".equalsIgnoreCase(orderItem.getPlayOptions())) {//赢
                ...
                if ("X".equalsIgnoreCase(orderItem.getPlayOptions())) {//赢
                    amount = betAmount.multiply(new BigDecimal(String.valueOf(orderItem.getHandleAfterOddsValue())).subtract(new BigDecimal(1))).multiply(new BigDecimal("-1"));
                } else {
                    amount = betAmount;
                }
                ...
                if ("2".equalsIgnoreCase(orderItem.getPlayOptions())) {//赢
                    amount = betAmount.multiply(new BigDecimal(String.valueOf(orderItem.getHandleAfterOddsValue())).subtract(new BigDecimal(1))).multiply(new BigDecimal("-1"));
                } else {
                    amount = betAmount;
                }
