...
/**
 * @description: 篮球 是否加时  Forecast计算
 * @author: lithan
 * @date:  2021-1-15 14:47:22
 **/
@Slf4j
@Service("basketMatrixIsOvertimeService")
public class BasketMatrixIsOvertimeServiceImpl implements BasketBallForecastService {
...
//                }
            if (i  == 0) {
                if ("Yes".equalsIgnoreCase(orderItem.getPlayOptions())) {//赢
...
//                    //走水
                ...
                if ("No".equalsIgnoreCase(orderItem.getPlayOptions())) {//赢
                    amount = betAmount.multiply(new BigDecimal(String.valueOf(orderItem.getHandleAfterOddsValue())).subtract(new BigDecimal(1))).multiply(new BigDecimal("-1"));
                } else {
                    amount = betAmount;
                }
