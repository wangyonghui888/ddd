...
public class Layout {
    ...
    public static class Table {
        ...
        public String toString() {
            ...
            int maxValueLength = rows.stream().map(Row::getValues).flatMap(Collection::stream).map(StrUtil::toString).map(StrUtil::length).max(Comparator.comparing(Integer::valueOf)).orElse(1) + 5;
