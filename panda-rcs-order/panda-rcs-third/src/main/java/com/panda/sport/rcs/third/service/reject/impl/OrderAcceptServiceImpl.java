package com.panda.sport.rcs.third.service.reject.impl;
...
import com.panda.sport.rcs.third.entity.common.RcsTournamentTemplateAcceptConfigDto;
...
import com.panda.sport.rcs.third.enums.SportIdEnum;
...
public class OrderAcceptServiceImpl implements IOrderAcceptService {
     ...
     * @return 是否滚球订单
     */
    public boolean orderIsScroll(List<ExtendBean> list) {
        //是否滚球
        boolean scrollFlag = false;
        for (ExtendBean item : list) {
            if ("1".equals(item.getIsScroll())) {
                log.info("::{}::当前订单存在滚球注单,需要等待处理{}", item.getOrderId(), JSONObject.toJSONString(item));
                return true;
            }
        }
        return scrollFlag;
                ...
                if (SportIdEnum.FOOTBALL.getId().equals(sportId) || SportIdEnum.BASKETBALL.getId().equals(sportId)) {
                    if (orderMarketId.equals(standardMarket.getId())) {
                        if (!standardMarket.getStatus().equals(0) || standardMarket.getThirdMarketSourceStatus() != 0) {
                            log.info("::{}::{}球种:{}盘口有变化拒单::盘口ID:{}盘口状态:{},订单状态:{}", orderNo, third, sportId, standardMarket.getId(),
                                    standardMarket.getStatus(), orderStatus);
                    ...
                    //盘口的位置有变化
                    if (StringUtils.equalsIgnoreCase(String.valueOf(standardMarket.getMarketCategoryId()), String.valueOf(tOrderDetail.getPlayId())) &&
                            StringUtils.equalsIgnoreCase(String.valueOf(standardMarket.getChildMarketCategoryId()), tOrderDetail.getSubPlayId()) &&
                            standardMarket.getMarketType().equals(matchType) &&
                            tOrderDetail.getItemBean().getPlaceNum().equals(standardMarket.getPlaceNum()) &&
                            !StringUtils.equalsIgnoreCase(String.valueOf(tOrderDetail.getMarketId()), String.valueOf(standardMarket.getId()))) {
     ...
     */
    @Override
    public Map<String, String> queryOddsRange(List<TOrderDetail> list, String third) {
        Map<String, String> oddsRange = new HashMap<>();
        String orderNo = list.get(0).getOrderNo();
        String defaultRange = "";
        try {
            for (TOrderDetail item : list) {
                //投注项id
