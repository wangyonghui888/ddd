...
@Singleton
@MatrixCacl(configs = "1:78;2:78")
public class Competitor1OddEven extends AbstractMatrix {

    /**
     * 玩法编码
     **/
    public static final String CATE_CODE = "78";
    ...
    public Integer getSettleResult(int m, int n, String templateCode) {
        ...
        switch (templateCode) {
            /*** 投奇数  ***/
            case "Odd":
                lostStatus = calculateOdd(m, n);
                break;
            /*** 投偶数 ***/
            case "Even":
                lostStatus = calculateEven(m, n);
                break;
            default:
     ...
     * @date 2020/3/17 9:52
     **/
    private Integer calculateOdd(int m, int n) {
     ...
     **/
    private Integer calculateEven(int m, int n) {
        /***  赢：N%2，余数=0 ***/
        if ((m % 2) == 0) {
            return OrderSettleStatus.USER_WIN.getValue();
        } else {
