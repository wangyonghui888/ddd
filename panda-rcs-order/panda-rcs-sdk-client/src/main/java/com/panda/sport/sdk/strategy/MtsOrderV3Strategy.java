 ...
 */
@Singleton
public class MtsOrderV3Strategy implements IOrderStrategy {

    private static final Logger logger = LoggerFactory.getLogger(MtsOrderV3Strategy.class);
    @Inject
    Producer producer;
    @Inject
    ...
    public Map<String, Object> checkOrder(OrderBean orderBean, MatrixForecastVo matrixForecastVo) {
        ...
        logger.info("::{}::额度查询-MTS，商户信息:{}", orderBean.getUid(), JSON.toJSONString(rcsQuotaBusinessLimit));
        ...
        if (orderBean.getItems().get(0).getMatchType() != 3) {
            ...
            if (orderBean.getSeriesType() == 1) {
                ...
                // 串关，先校验pa额度
                OrderCheckResultVo checkResult = seriesLimitService.checkBetAmountAndPayment(orderBean, type,rcsQuotaBusinessLimit);
        ...
        mtsBetTicket(orderBean, matrixForecastVo);
     ...
     **/
    private void mtsBetTicket(OrderBean orderBean, MatrixForecastVo matrixForecastVo) {
...
//                paramValidateService.putBeanVal(bean, matrixForecastVo);
        ...
        producer.sendMsg(MqConstants.RCS_VALIDATE_MTS_ORDER, MqConstants.RCS_VALIDATE_MTS_ORDER_TAG, orderBean.getOrderNo(), JSONObject.toJSONString(map), orderBean.getOrderNo());
    }
...
//		BetAmountLimitVo betAmountLimitVo = limitConfigService.getBetAmountLimit(extendBeanList.get(0));
        ...
        //获取商户信息
        RcsQuotaBusinessLimitResVo rcsQuotaBusinessLimit = limitConfigService.getBusinessLimit(Long.valueOf(orderBean.getExtendBean().getBusId()));
        logger.info("::{}::投注-MTS，商户信息:{}", orderBean.getOrderNo(), JSON.toJSONString(rcsQuotaBusinessLimit));
        //设置最小值
        ...
        if (seriesType != 1) {
            ...
            if (UserSpecialLimitType.VIP.getType().equals(type)) {
                ...
                paList = seriesLimitService.queryMaxBetMoneyBySelect(orderBean, true,rcsQuotaBusinessLimit);
            }
            ...
            for (int i = 2; i <= seriesNum; i++) {
                ...
                try {
                    ...
                    if ("0".equals(mtsCircuitTag)) amount = mtsApiService.getMaxStake(request).getData();
                    else amount = 2000L;
                } catch (Exception e) {
            ...
            if (seriesNum > 2) {
                //增加N串M的额度
                Request<MtsgGetMaxStakeDTO> request = new Request<MtsgGetMaxStakeDTO>();
            ...
            logger.info("获取mts单注最大限额返回" + JSONObject.toJSONString(list));
        } else { //单注
            Request<ExtendBean> request = new Request<ExtendBean>();
            request.setGlobalId(MDC.get("X-B3-TraceId") + seriesType);
            firstExtendBean.getItemBean().setBetAmount(10000L);
            request.setData(firstExtendBean);
            try {
                String mtsCircuitTag = getMtsCacheTag();
                if ("0".equals(mtsCircuitTag)) amount = mtsApiService.getSingleMaxStake(request).getData();
                else amount = 2000L;
            } catch (Exception e) {
                logger.info("获取mts额度异常{},{}", e.getMessage(), e);
                amount = 2000L;
            }
            logger.info("额度查询-获取mts单注最大限额:{},requestId:{}", amount, request.getGlobalId());
            vo.setOrderMaxPay(amount);
            vo.setMinBet(0L);
