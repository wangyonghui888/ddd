...
@Slf4j
@Service
public class TennisScoreService extends AbstractScoreService {
...
/*        unFilterScoreData.add("breakSuccessCount-0-0");
            ...
            while (iterator.hasNext()) {
                ...
                if("setScore".equals(matchStatisticsInfoDetail.getCode())){
                    ...
                    if(null==data.getSecondNum()){
                        ...
                        log.info("::{}::Tennis-score,matchId{},period{},secondNum{}",data.getStandardMatchId(),period,data.getSecondNum());
                    }
                }
                if (period.equals("-1")) {
                    matchStatisticsInfoDetail.setFirstNum(0);
    ...
    private void buildWsScoreVO(Map value, WsScoreVO wsScoreVO, String period, String key, Integer secondNum, Request<StandardScoreDto> request) {
        try {
            ...
            } else if ("qiangScore".equals(key)) {
                wsScoreVO.setQiangScore(score);
            } else if ("currentScore".equals(key)) {
                String home =Integer.valueOf(String.valueOf(value.get("home")))>40?"A":String.valueOf(value.get("home"));
                String away =Integer.valueOf(String.valueOf(value.get("away")))>40?"A":String.valueOf(value.get("away"));
                score = home + ":" + away;
                wsScoreVO.setCurrentScore(score);
            }
        } catch (Exception e) {
            log.error("::{}::{},{},{}","RDSMSG_"+request.getLinkId(),JsonFormatUtils.toJson(value),e.getMessage(), e);
        }
    ...
    protected MatchPeriod getMatchPeriod(List<MatchStatisticsInfoDetail> matchStatisticsInfoDetails, Long standardMatchId, Integer periodId, String linkId) {
        try {
            ...
            if (!CollectionUtils.isEmpty(matchStatisticsInfoDetails)) {
                ...
                if (800 == periodId.intValue() || 301 == periodId.intValue()) {
                    matchPeriod.setPeriod(8);
                } else if (900 == periodId.intValue() || 302 == periodId.intValue()) {
                    matchPeriod.setPeriod(9);
                } else if (1000 == periodId.intValue() || 303 == periodId.intValue()) {
                    matchPeriod.setPeriod(10);
                } else if (1100 == periodId.intValue() || 304 == periodId.intValue()) {
                    matchPeriod.setPeriod(11);
                } else if (1200 == periodId.intValue()) {
