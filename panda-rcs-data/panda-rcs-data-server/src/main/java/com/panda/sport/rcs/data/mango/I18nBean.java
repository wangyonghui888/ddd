package com.panda.sport.rcs.data.mango;
...
public class I18nBean implements Serializable {
    ...
    public void replaceTeam(Map<String, I18nBean> teamMap) {
        if (CollectionUtils.isEmpty(teamMap)) {
            ...
            teamMap.put(RcsConstant.HOME_POSITION, BeanFactorys.getHomeTeam());
            teamMap.put(RcsConstant.AWAY_POSITION, BeanFactorys.getAwayTeam());
        }
        I18nBean homeTeam = teamMap.get(RcsConstant.HOME_POSITION);
        if (homeTeam == null) {
            homeTeam = BeanFactorys.getHomeTeam();
        }
        I18nBean awayTeam = teamMap.get(RcsConstant.AWAY_POSITION);
        if (awayTeam == null) {
            awayTeam = BeanFactorys.getAwayTeam();
        }
    ...
    public void replaceOverAndUnder() {
        replace(Placeholder.OVER, BeanFactorys.getOverI18n());
        replace(Placeholder.UNDER, BeanFactorys.getUnderI18n());
    }

    public void replaceTitle(Map<String, I18nBean> teamMap) {
        replace(Placeholder.DRAW, BeanFactorys.getDrawI18n());
        replace(Placeholder.OTHER, BeanFactorys.getOtherI18n());
        replaceOverAndUnder();
        replaceTeam(teamMap);
    }
    ...
    public static I18nBean getHome(Map<String, I18nBean> teamMap) {
        ...
        I18nBean bean = teamMap.getOrDefault(RcsConstant.HOME_POSITION, BeanFactorys.getHomeTeam());
    ...
    public static I18nBean getAway(Map<String, I18nBean> teamMap) {
        ...
        I18nBean bean = teamMap.getOrDefault(RcsConstant.AWAY_POSITION, BeanFactorys.getAwayTeam());
        if (StringUtils.isBlank(bean.getZs()) && StringUtils.isNotBlank(bean.getEn())) {
            bean.setZs(bean.getEn());
        }
        return bean;
