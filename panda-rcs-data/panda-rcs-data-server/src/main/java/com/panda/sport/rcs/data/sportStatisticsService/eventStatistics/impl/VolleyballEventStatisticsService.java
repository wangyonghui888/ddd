...
@Slf4j
@TraceCrossThread
public class VolleyballEventStatisticsService extends AbstractEventStatisticsService {


    private static final String VOLLEYBALL_TIME_CORRECTION = "volleyballTimeCorrection";
    ...
    protected void initial() {
        sportId = 9L;
        mongoEvents = Arrays.asList("match_status","volleyball_score_change");
    ...
    public Map standardScore(Request<MatchEventInfoMessage> request,int size) {
        try {
            ...
            //网球局暂停
            boolean volleyballPeriodJuge = false;
            ...
            /*** 如果mongo事件进行发送***/
            volleyballPeriodJuge = StringUtils.isNotBlank(oldPeriod)&& StandardMatchInfoServiceImpl.TENNIS_SET_PUASE.contains(oldPeriod);
            String ifPresent = cache.getIfPresent(String.valueOf(data.getStandardMatchId()));
            if (StringUtils.isBlank(ifPresent) || mongoEvents.contains(data.getEventCode())||volleyballPeriodJuge) {
    ...
    protected MatchPeriod getMatchPeriod(Long sportId, MatchEventInfoMessage data,String linkId) {
        ...
        if (data != null) {
            ...
            //排球特殊处理 ---- start ----如果网球缓存有timeout 则不下发事件code到mongo
            String eventFlag = redisClient.get(String.format(RCS_DATA_KEY_CACHE_KEY, VOLLEYBALL_TIME_CORRECTION, data.getStandardMatchId()));
            if (StringUtils.isNotBlank(eventFlag) && "timeout".equals(eventFlag)) {
                isSetEventCode = false;
                log.info("::{}::乒乓球timout-flag-2:{}", "RDMEITG_"+linkId+"_"+data.getStandardMatchId(),data.getStandardMatchId());
            }
            //排球特殊处理 ---- end
            if (isSetEventCode) {
