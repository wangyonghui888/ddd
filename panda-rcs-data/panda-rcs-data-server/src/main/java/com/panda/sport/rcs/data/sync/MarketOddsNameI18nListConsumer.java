...
@RocketMQMessageListener(
        topic = "MARKET_ODDS_NAME_I18N_LIST",
        consumerGroup = "RCS_DATA_MARKET_ODDS_NAME_I18N_LIST_GROUP",
        messageModel = MessageModel.CLUSTERING,
        consumeMode = ConsumeMode.CONCURRENTLY)
public class MarketOddsNameI18nListConsumer extends RcsConsumer<Request<List<MarketOddsNameI18nList>>> {
    ...
    protected String getTopic() {
        return "MARKET_ODDS_NAME_I18N_LIST";
    }
            ...
            if(!CollectionUtils.isEmpty(rcsLanguageInternations2)){
                ...
                for (String nameCode : collect1.keySet()) {
                    ...
                    if(CollectionUtils.isEmpty(list2)){
                        ...
                        log.info("::{}::MARKET_ODDS_NAME_I18N_LIST 数据商下发数据 nameCode已经存在 不存","RDMONILG_"+nameCode);
                    }
                }
            }
        } catch (Exception e) {
            log.error("::{}::{},{},{}","RDMONILG_"+rRequests.getLinkId(),JsonFormatUtils.toJson(rRequests),e.getMessage() , e);
        }
        return true;
    }
