...
@Slf4j
@TraceCrossThread
public class PingPongEventStatisticsService extends AbstractEventStatisticsService {


    private static final String PINGPONG_TIME_CORRECTION = "pingPongTimeCorrection";
    ...
    protected void initial() {
        sportId = 8L;
        mongoEvents = Arrays.asList("match_status","table_tennis_score_change");
    ...
    protected MatchPeriod getMatchPeriod(Long sportId, MatchEventInfoMessage data,String linkId) {
        ...
        if (data != null) {
            ...
            //乒乓球特殊处理 ---- start ----如果羽毛球缓存有timeout 则不下发事件code到mongo
            String eventFlag = redisClient.get(String.format(RCS_DATA_KEY_CACHE_KEY, PINGPONG_TIME_CORRECTION, data.getStandardMatchId()));
