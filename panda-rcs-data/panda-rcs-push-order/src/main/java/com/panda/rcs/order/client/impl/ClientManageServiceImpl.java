package com.panda.rcs.order.client.impl;
...
import com.panda.rcs.order.entity.enums.LanguageEnums;
...
import com.panda.rcs.order.entity.vo.SingleSubInfoVo;
...
public class ClientManageServiceImpl implements ClientManageService {
    ...
    public void sendMessage(SubscriptionEnums subscriptionEnums, List<OrderBetResponseVO> orderBetList, Object enMessage, Object zsMessage) {
        log.info("::Socket客户端，连接信息->筆數{}::{}", ClientCache.clientGroupMap.size(), JSONObject.toJSONString(ClientCache.clientGroupMap));
        if (ClientCache.clientGroupMap.size() > 0) {
            ...
            if (validClientMap.size() > 0) {
                log.info("::validClientMap，信息->::{}", JSONObject.toJSONString(validClientMap));
                ...
                log.info("::英文订阅客户端组，信息::{}", JSONObject.toJSONString(enPushClientGroup));
                ...
                log.info("::中文订阅客户端组，信息::{}", JSONObject.toJSONString(zsPushClientGroup));
    ...
    private static void commSendMessage(DefaultChannelGroup channelGroup, Object message) {
       ...
       log.info("socket commSendMessage {}",JSONObject.toJSONString(message));
