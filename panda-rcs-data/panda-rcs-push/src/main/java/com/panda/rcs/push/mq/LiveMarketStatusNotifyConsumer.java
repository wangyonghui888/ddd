...
import com.panda.sport.rcs.wrapper.*;
...
/**
 * @Description: 标准赔率推送第二版本
 * Topic=STANDARD_MARKET_ODDS_PUSH_WS
 * Group=RCS_WS_PUSH_STANDARD_MARKET_ODDS_GROUP
 * 对应指令-> 30005
 */
@Slf4j
@Component
...
@RocketMQMessageListener(
        topic = "STANDARD_MARKET_ODDS_PUSH_WS",
        consumerGroup = "RCS_WS_PUSH_STANDARD_MARKET_ODDS_GROUP",
        messageModel = MessageModel.CLUSTERING,
        consumeMode = ConsumeMode.CONCURRENTLY)
public class LiveMarketStatusNotifyConsumer implements RocketMQListener<String>, RocketMQPushConsumerLifecycleListener {
    ...
    public void onMessage(String message) {
        ...
        Request<LiveStandardMatchMarketMessageVO> msg = null;
        try {
            message = JSON.parse(message).toString();
            msg = JSON.parseObject(message, new TypeReference<Request<LiveStandardMatchMarketMessageVO>>() {});
            if (!msgCheck(msg)) {
                return;
            }
            String linkId = msg.getLinkId();
            LiveStandardMatchMarketMessageVO matchMarketMessageVO = msg.getData();
            log.info("::{}::STANDARD_MARKET_ODDS_PUSH_WS标准赔率推送->赛事id{}", linkId, matchMarketMessageVO.getStandardMatchInfoId());
            Long dataSourceTime = msg.getDataSourceTime();
            ...
            for (LiveStandardMarketMessageVO market : newList) {
                ...
                //2384 数据站点
                if("LS".equals(market.getDataSourceCode())&& StringUtils.isNotEmpty(market.getInternalDataSourceCode())){
                    market.setDataSourceCode(market.getInternalDataSourceCode());
                }
            }
            ...
            log.info("::{}::赛事标准状态处理后数据->赛事id：{},玩法：{}", linkId,matchMarketMessageVO.getStandardMatchInfoId(), JSON.toJSONString(playIds));
            ...
            log.error("::{}::赛事标准赔率消费数据，异常信息：", "STANDARD_MARKET_ODDS_PUSH_WS", e);
        }
    }
