...
public class ProducerSendMessageUtils {
    ...
    public void sendMsg(String topic,String tags, String key, String data,HashMap<String, String> properties ,String hashKey) {
        try {
            ...
            log.info("发送消息：topic：{}，tag：{}，key：{}，data：{}，result:{}，map：{}",topic,tags,key,data,JSONObject.toJSONString(result),JSONObject.toJSONString(msg.getProperties()));
        } catch (Exception e) {
        	log.error(e.getMessage(),e);
        }
    }
    ...
    public void sendMessage(Object obj ,MessageQueue messageQueue )  {
        try {
        	log.info("发送消息：{}，指定队列:{}",JSONObject.toJSONString(obj),JSONObject.toJSONString(messageQueue));
