...
public class LogModifyMargainRefFormat extends LogFormatStrategy {
    ...
    private void configDiffFilter(RcsOperateLog rcsOperateLog, LogAllBean newParam) {
        //取原始值
        LogAllBean oriVo = BaseUtils.mapObject(newParam.getBeforeParams(),LogAllBean.class) ;
        ...
        if (Objects.isNull(newParam.getId())) {
            RcsOperateLog templateLog = prepareLogBean(rcsOperateLog, OperateLogEnum.NONE.getName(), OperateLogOneEnum.ADD.getName());
            //templateLog.setParameterName(playName + "-" + timeValName);
            templateLog.setParameterName(timeValName);
        ...
        if (Objects.nonNull(newParam.getMargain()) &&
            ...
            templateLog.setParameterName(OperateLogEnum.SPECIAL_ODDS_INTERVAL.getName());
            pushMessage(templateLog);
        }
        //累计差额计算方式
        if (Objects.nonNull(newParam.getBalanceOption()) &&
            ...
            templateLog.setParameterName(OperateLogEnum.BALANCE_OPTION.getName());
            pushMessage(templateLog);
        }
        ...
        if (Objects.nonNull(newParam.getMinOdds()) &&
            ...
            templateLog.setParameterName(OperateLogEnum.MIN_MALAY_ODDS.getName());
            pushMessage(templateLog);
        }
        ...
        if (Objects.nonNull(newParam.getMaxOdds()) &&
            ...
            templateLog.setParameterName(OperateLogEnum.MAX_MALAY_ODDS.getName());
            pushMessage(templateLog);
        }
        ...
        if (Objects.nonNull(newParam.getOddChangeRule()) &&
            ...
            templateLog.setParameterName(OperateLogEnum.ODD_CHANGE_RULE.getName());
            pushMessage(templateLog);
        }
        ...
        if (Objects.nonNull(newParam.getHomeMultiMaxAmount()) &&
            ...
            templateLog.setParameterName(OperateLogEnum.HOME_MULTI_MAX_AMOUNT.getName());
            pushMessage(templateLog);
        }

        if(Objects.nonNull(newParam.getMultiDiffVal())&& !newParam.getMultiDiffVal().equals(oriVo.getMultiDiffVal())){
            RcsOperateLog templateLog = prepareLogBean(rcsOperateLog, oriVo.getMultiDiffVal(), newParam.getMultiDiffVal());
            templateLog.setParameterName(OperateLogEnum.MAX_AMOINT.getName());
            pushMessage(templateLog);
        } if(Objects.nonNull(newParam.getMultiOddsRate())&& !newParam.getMultiOddsRate().equals(oriVo.getMultiOddsRate())){
            RcsOperateLog templateLog = prepareLogBean(rcsOperateLog, oriVo.getMultiOddsRate(), newParam.getMultiOddsRate());
            templateLog.setParameterName(OperateLogOneEnum.Cumulative_probability_change.getName());
            pushMessage(templateLog);
        }
        if(Objects.nonNull(newParam.getPendingOrderPayVal())&& !newParam.getPendingOrderPayVal().equals(oriVo.getPendingOrderPayVal())){
            RcsOperateLog templateLog = prepareLogBean(rcsOperateLog, oriVo.getPendingOrderPayVal(), newParam.getPendingOrderPayVal());
            templateLog.setParameterName(OperateLogOneEnum.Accumulated_reserved_compensation.getName());
            pushMessage(templateLog);
        }
        ...
        if (Objects.nonNull(newParam.getHomeMultiOddsRate()) &&
            ...
            templateLog.setParameterName(OperateLogEnum.HOME_MULTI_ODDS_RATE.getName());
            pushMessage(templateLog);
        }
        ...
        if (Objects.nonNull(newParam.getAwayMultiOddsRate()) &&
            ...
            templateLog.setParameterName(OperateLogEnum.AWAY_MULTI_ODDS_RATE.getName());
            pushMessage(templateLog);
        }
        ...
        if (Objects.nonNull(newParam.getHomeSingleMaxAmount()) &&
            ...
            templateLog.setParameterName(OperateLogEnum.HOME_SINGLE_MAX_AMOUNT.getName());
            pushMessage(templateLog);
        }
        ...
        if (Objects.nonNull(newParam.getHomeSingleOddsRate()) &&
            ...
            templateLog.setParameterName(OperateLogEnum.HOME_SINGLE_ODDS_RATE.getName());
            pushMessage(templateLog);
        }
        ...
        if (Objects.nonNull(newParam.getAwaySingleOddsRate()) &&
            ...
            templateLog.setParameterName(OperateLogEnum.AWAY_SINGLE_ODDS_RATE.getName());
            pushMessage(templateLog);
        }
        ...
        if (Objects.nonNull(newParam.getHomeLevelFirstMaxAmount()) &&
            ...
            templateLog.setParameterName(OperateLogEnum.HOME_LEVEL_FIRST_MAX_AMOUNT.getName());
            pushMessage(templateLog);
        }
        ...
        if (Objects.nonNull(newParam.getHomeLevelFirstOddsRate()) &&
            ...
            templateLog.setParameterName(OperateLogEnum.HOME_LEVEL_FIRST_ODDS_RATE.getName());
            pushMessage(templateLog);
        }
        ...
        if (Objects.nonNull(newParam.getAwayLevelFirstOddsRate()) &&
            ...
            templateLog.setParameterName(OperateLogEnum.AWAY_LEVEL_FIRST_ODDS_RATE.getName());
            pushMessage(templateLog);
        }
        ...
        if (Objects.nonNull(newParam.getHomeLevelSecondMaxAmount()) &&
            ...
            templateLog.setParameterName(OperateLogEnum.HOME_LEVEL_SECOND_MAX_AMOUNT.getName());
            pushMessage(templateLog);
        }
        ...
        if (Objects.nonNull(newParam.getHomeLevelSecondOddsRate()) &&
            ...
            templateLog.setParameterName(OperateLogEnum.HOME_LEVEL_SECOND_ODDS_RATE.getName());
            pushMessage(templateLog);
        }
        ...
        if (Objects.nonNull(newParam.getAwayLevelSecondOddsRate()) &&
            ...
            templateLog.setParameterName(OperateLogEnum.AWAY_LEVEL_SECOND_ODDS_RATE.getName());
            pushMessage(templateLog);
        }
        ...
        if (Objects.nonNull(newParam.getOrderSinglePayVal()) &&
            ...
            templateLog.setParameterName(OperateLogEnum.ORDER_SINGLE_PAY_VAL.getName());
            pushMessage(templateLog);
        }
        ...
        if (Objects.nonNull(newParam.getUserMultiPayVal()) &&
            ...
            templateLog.setParameterName(OperateLogEnum.USER_MULTI_PAY_VAL.getName());
            pushMessage(templateLog);
        }
        ...
        if (Objects.nonNull(newParam.getCategoryPreStatus()) &&
            ...
            templateLog.setParameterName(OperateLogEnum.CATEGORY_PRE_STATUS.getName());
            pushMessage(templateLog);
        }
        //CashOut Margin
        if (Objects.nonNull(newParam.getCashOutMargin()) &&
            ...
            templateLog.setParameterName(OperateLogEnum.CASH_OUT_MARGIN.getName());
     ...
     */
    @SneakyThrows
    public void configPingPongLogFormat(RcsOperateLog rcsOperateLog, LogAllBean param) {
        ...
        //取历史值
        LogAllBean beforeParam = BaseUtils.mapObject(param.getBeforeParams(),LogAllBean.class) ;
        //查詢玩法名稱
        ...
        if(param != null){
            ...
            if (!beforeStr.equals(newStr)) {
                String beforeVal = beforeIsAutoClose.equals(0) ? "不限" : beforeScore;
                rcsOperateLog.setBeforeVal(beforeVal);
                String afterVal = param.getIsAutoCloseScoreConfig().compareTo(0) == 0 ? "不限" : param.getAchieveCloseScore().toString();
        ...
        if (param.getMatchManageId() == null){
            ...
            rcsOperateLog.setObjectName(montageEnAndZsIs(param.getTeamList(),param.getMatchId()));
        }
     ...
     * @param param
     */
    private void initalTemplateFormat(RcsOperateLog rcsOperateLog, LogAllBean param) {
        ...
        rcsOperateLog.setExtObjectNameByObj(getPlayNameZsEn(param.getPlayId().longValue(), param.getSportId()));
        rcsOperateLog.setBehavior(OperateLogEnum.TEMPLATE_UPDATE.getName());
    }
     ...
     * @param param
     */
    private void initalOperateFormat(RcsOperateLog rcsOperateLog, LogAllBean param) {
