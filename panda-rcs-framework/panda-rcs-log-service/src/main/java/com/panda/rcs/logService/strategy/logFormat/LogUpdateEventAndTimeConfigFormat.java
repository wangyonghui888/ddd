...
import com.panda.rcs.logService.vo.RcsTournamentTemplateAcceptEvent;
...
public class LogUpdateEventAndTimeConfigFormat extends LogFormatStrategy {
    ...
    private void updateEventConfigFormat(RcsOperateLog sample,LogAllBean config) {
        ...
        Map<String, List<LogAllBean>> eventTypeMap = config.getEvents().stream().collect(groupingBy(LogAllBean::getEventType));
        List<String> newSafetyList = eventTypeMap.getOrDefault("safety", new ArrayList<>()).stream().map(LogAllBean::getEventName).collect(Collectors.toList());
        List<String> newDangerList = eventTypeMap.getOrDefault("danger", new ArrayList<>()).stream().map(LogAllBean::getEventName).collect(Collectors.toList());
        List<String> newClosingList = eventTypeMap.getOrDefault("closing", new ArrayList<>()).stream().map(LogAllBean::getEventName).collect(Collectors.toList());
        List<String> newRejectList = eventTypeMap.getOrDefault("reject", new ArrayList<>()).stream().map(LogAllBean::getEventName).collect(Collectors.toList());
    ...
    private void initialLogBean(RcsOperateLog rcsOperateLog, LogAllBean config) {
        ...
        rcsOperateLog.setObjectNameByObj(templateIdEnAndZsIs(config.getTemplateId(),config.getSportId()));
     ...
     * @return
     */
    private List<String> getOriEventNameList(LogAllBean config, String eventType) {
